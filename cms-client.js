...
function safeProperty(item) { return {category:'standard',transaction:'Sale',units:[],...safeValue(item)}; }
...
async function listingAction(action,item) {
  ...
  if(action==='status'){
    const statuses=item.transaction==='Rent'?['Available','Rented','Draft']:['Available','Sold','Draft'];
    const d=modal('Disponibilité','<label>Statut<select id="status-choice">'+statuses.map(s=>`<option value="${s}" ${s===item.status?'selected':''}>${statusLabel(s)}</option>`).join('')+'</select></label><button class="primary-button" id="confirm-status">Enregistrer le statut</button>');
...
function collectEditor(form,mode='changes') {
  ...
  return {...old,...values,id:id||undefined,price:values.price,
    rentalPeriod:values.transaction==='Rent'?values.rentalPeriod:null,
    minRentalMonths:values.transaction==='Rent'?values.minRentalMonths:null,
...
function bindEditor() {
  ...
  const updateTransactionFields=()=>{
    const rental=form.elements.transaction.value==='Rent';
    $$('.rental-only',form).forEach(field=>{field.hidden=!rental;});
    for(const option of form.elements.status.options){
      const inappropriate=rental?option.value==='Sold':option.value==='Rented';
      option.hidden=inappropriate;option.disabled=inappropriate;
    }
    if(form.elements.status.selectedOptions[0]?.disabled)form.elements.status.value='Available';
    $('#property-price-caption',form).textContent=rental?'Prix de location (optionnel)':'Prix (optionnel)';
    $$('[data-unit-field="price"]',form).forEach(input=>{input.closest('label').firstChild.textContent=rental?'Prix de location (optionnel)':'Prix (optionnel)';});
    $$('[data-unit-field="status"]',form).forEach(select=>{
      for(const option of select.options){
        const inappropriate=rental?option.value==='Sold':option.value==='Rented';
        option.hidden=inappropriate;option.disabled=inappropriate;
      }
      if(select.selectedOptions[0]?.disabled)select.value=rental?'Rented':'Sold';
    });
  };
  form.elements.transaction.addEventListener('change',()=>{updateTransactionFields();cms.dirty=true;});
  ...
  const unitCard=(unit={})=>{
    ...
    card.innerHTML=`<div class="unit-card-head"><strong>Unité</strong><div><button type="button" data-unit-action="up" aria-label="Déplacer l’unité vers le haut">↑</button><button type="button" data-unit-action="down" aria-label="Déplacer l’unité vers le bas">↓</button><button type="button" data-unit-action="remove" aria-label="Supprimer l’unité">×</button></div></div><div class="unit-fields"><label class="unit-name">Nom / Référence de l’unité<input data-unit-field="name" value="${escapeHtml(unit.name||'')}" maxlength="100" placeholder="Ex. : Appartement A-01" required></label><label>Surface (m²)<input data-unit-field="area" type="number" min="0" value="${unit.area??''}"></label><label>Prix<input data-unit-field="price" inputmode="decimal" value="${unit.price??''}"></label><label>Chambres<input data-unit-field="beds" type="number" min="0" value="${unit.beds??''}"></label><label>Étage<input data-unit-field="floor" type="number" min="0" value="${unit.floor??''}"></label><label>Statut<select data-unit-field="status"><option value="Available" ${!['Sold','Rented','Reserved'].includes(unit.status)?'selected':''}>Disponible</option><option value="Sold" ${unit.status==='Sold'?'selected':''}>Vendu</option><option value="Rented" ${unit.status==='Rented'?'selected':''}>Loué</option><option value="Reserved" ${unit.status==='Reserved'?'selected':''}>Réservé</option></select></label></div><details class="unit-images"><summary>Photos spécifiques (<span>0</span>)</summary><div class="unit-image-options"></div><small>Facultatif — choisissez des photos déjà ajoutées à la galerie du bien.</small></details>`;
    const priceField=$('[data-unit-field="price"]',card);
    priceField.placeholder='Laisser vide si non défini';priceField.closest('label').firstChild.textContent='Prix (optionnel)';
    return card;
  };
  refreshUnitImages=()=>{
  ...
  refreshUnitImages();updateUnitCount();updateTransactionFields();
  $('#add-unit',form).onclick=()=>{if($$('.unit-editor-card',unitList).length>=50){formError(form,'Vous pouvez ajouter au maximum 50 unités.');return;}unitList.append(unitCard());refreshUnitImages();updateUnitCount();updateTransactionFields();cms.dirty=true;};
